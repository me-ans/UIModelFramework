...
class UIEditor  : public WindowUIModel
{
public:
    
#include "ans_UIEditor.specs.h"
    
    /** Default constructor. Both arguments may be null, allowing the user to select them  */
    UIEditor (UIModel::Class* client = nullptr, UISpec* spec = nullptr);
   ~UIEditor ();
    ...
    void updateSpecSelection()
    {
        if (getSelectedUISpec() != nullptr)
            componentTree.setRoot (getSelectedUISpec()->getRootSpec());
        else
    ...
    {
        inspector->setSelectedSpec (componentTree.getSingleSelection());
    ...
    /** Get the UISpec being edited - CAUTION: May be null */
    UISpec* getSelectedUISpec () { return specList.getSingleSelection(); }
